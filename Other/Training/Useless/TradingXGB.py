...
global_c = 0.5 # the higher, the more the model predicts neutral
global_c2 = 0.5 #the lower, the more the model predicts neutral

def count_neutral (y):
    return np.float(np.count_nonzero(y[:,1] == 1)) / len(y)
...
def getSetsFromDataset (ds, bConvolveCdl=False, mm_200_idx=8, bNormalize_by_mm200=True, bShuffleTrainset = True):
    if len(np.shape(ds.X))==3:
        X = ds.X[:,-1,:]
        y = np.dot(ds.y, [0,1,2])
    else:
        X = []
        y = []
    if len(np.shape(ds.cv_X))==3:
        cv_X = ds.cv_X[:,-1,:]
        cv_y = np.dot(ds.cv_y, [0,1,2])
    else:
        cv_X = []
        cv_y = []
    if len(np.shape(ds.test_X))==3:
        test_X = ds.test_X[:,-1,:]
...
series_list2 = [302, 303, 304, 305, 306, 307, 308, 309, 310, 311, 312, 313, 314, 315]
...
if False:
    ...
    #feat_sel = [16,17,18,19,20,22,70,71,75,78,152,153,179,180,181,182,183,184,185,186,187,188,189,190,191,192,193,194, 81, 84, 85, 86, 168,169,170,171,172,173,174,175,176,177,178, -3, -2, -1]
    feat_sel = [15,16,17,21,81,191]
    ...
    t_xgb = TradingXGB(modelname='trees', dataset=ds, param=param, num_trees=5, c1=0.0, c2=1.0, pred_threshold=0.40)
    ...
    for i, test_set_size in enumerate(set_size_list[0:1]):
        ...
        #ds.loadSeriesByNo (series_no, bRelabel=False, bNormalize=False, bConvolveCdl=True)
        ds.loadDataSet(series_list=series_list, begin=1, end=10, bRelabel=False, bNormalize=False)
