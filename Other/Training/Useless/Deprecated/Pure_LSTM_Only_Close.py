...
modelname = 'Pure_LSTM_Only_1Layer_Only_Close'
...
class PredictAheadModel (TradingModel):
    def keepOnlyClosePrice (self):
        #training data
        X = self.dataset.X
        sh = np.shape(X)
        new_X = np.zeros((sh[0], sh[1], 1))
    ...
    def predict5daysAheadByNo (self, series_no):
    ...
    def buildModel (self):
        ...
        self.model.add(LSTM(2, stateful=False, return_sequences=False))
        #self.model.add(Dropout(0.2))
...
my_train = PredictAheadModel (modelname=modelname,
                            modelpath=modelpath, n_features=1, 
                            lookback_window=252, cv_set_size=500,
                            test_set_size=2000)
...
for i in range (0):
    my_train.modelname = 'Pure_LSTM_Only_1Layer_Only_Close'
    f = open (logpath+my_train.modelname,'a')
    random.shuffle(series_list)
    my_train.dataset.min_period_ahead = 5
    my_train.dataset.max_period_ahead = 10
    my_train.dataset.bCenter_y = True
    try:
        ...
        try:
            ...
            if True:
                if True:                
                    ...
                    k = 0
                    my_train.loadDataSet(series_list=series_list,begin=0, end=80)
                    my_train.createSingleTrainSet()
                    my_train.dataset.dataset_list = []

                    my_train.keepOnlyClosePrice ()
