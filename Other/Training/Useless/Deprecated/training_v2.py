...
print('Build model...')
model = Sequential()
model.add(Dense(output_dim=128, dropout=0.2, batch_input_shape=(32,14)))
model.add(LSTM(128, stateful=True, dropout_W=0.2, dropout_U=0.2))  # try using a GRU instead, for fun
model.add(Dense(3))
...
for series_no in range (1, no_series+1,1):
    ...
    X = np.zeros((len(sentences), 14))
    y = np.zeros((len(sentences),len(chars)), dtype=np.bool)
    test_X = np.zeros((len(test_sentences), 14))
