...
def compute_return (model, X, y, threshold=0.6):
    #try:
    if True:
        pred_prob = model.predict(X, batch_size=128)
...
class My_Training ():
    def __init__ (self, modelname, modelpath="./", featpath="./", parsedpath="./", labelpath="./",
                  lookback_window=252, n_features=75, cv_set_size=1000, test_set_size=1000, pred_threshold=0.7,
    ...
    def compute_return (self, dataset='cv'):
        if dataset=='cv':
            ...
            X = self.test_X
            y = self.test_y
        return compute_return(self.model, X, y, threshold=self.pred_threshold)
    ...
    def loadDataSetV2 (self,series_list=None, begin=0,end=10):
        ...
        for counter, series_no in enumerate(series_list[begin:end]):
            ...
            elem = [self.loadSeriesByNo (series_no, bLoadOnlyTrainset=True)]
            #self.dataset_list.append (elem)
            
            print ('Len aux_X: '+str(len(aux_X)))
            
            try:
                self.X = np.zeros ((len(elem[0][0]) + len (aux_X), np.size(aux_X,1), np.size(aux_X,2) ) )
                self.y = np.zeros ((len(elem[0][1]) + len (aux_y), np.size(aux_y,1) ) )
                
                print ('Len X: '+str(len(self.X)))
                
                self.X [0:len(aux_X),:,:] = aux_X
                self.y [0:len(aux_X),:] = aux_y
                self.X [len(aux_X):,:,:] = elem[0][0]
                self.y [len(aux_X):,:] = elem[0][1]
            except:
    ...
    def loadDataSet (self,series_list=None, begin=0,end=10):
        ...
        for counter, series_no in enumerate(series_list[begin:end]):
            elem = [self.loadSeriesByNo (series_no)]
            self.dataset_list.append (elem)
        return
    ...
    def buildModel (self):
        ...
        model = Sequential()
        model.add(LSTM(512, dropout_W=0.2, dropout_U=0.2, stateful=False, return_sequences=True, input_shape=(self.lookback_window, self.n_features)))
        #model.add(Dropout(0.5))
        model.add(LSTM(512, dropout_W=0.2, dropout_U=0.2, return_sequences=False))
        model.add(Dropout(0.5))
    ...
    def evaluateOnLoadedDataset (self, show_plots=True):
        ...
        for k in range(len (self.dataset_list)):
            if np.size(self.dataset_list[k],1) == 6:                    
                ...
                self.ret_array[k], self.long_hits_array[k], self.long_misses_array[k], self.short_hits_array[k], self.short_misses_array[k] = compute_return(self.model,self.cv_X, self.cv_y)
        if show_plots == True:
            fig = plt.figure ()
            #for k in range(len (self.dataset_list)):
    ...
    def createSingleTrainSet (self):
       ...
       self.X = np.zeros((total_len, self.lookback_window, self.n_features))
    ...
    def loadSeriesByNo (self, series_no, bLoadOnlyTrainset=False):
        ...
        try:
            ...
            sentences, next_chars = buildSentences (train_df, train_labels_df, bZeroLongDatedMovingAverages=self.bZeroMA_train)
...
def buildSequencePatches (sentences, next_chars, lookback_window=252):
...
my_train = My_Training (model_name, model_path, lookback_window=lookback_window)
