...
modelname = 'Pure_LSTM_Only_Px_No_Bias_v2'
...
class PredictAheadModel (TradingModel):
    ...
    def dropVolumeData (self):
        #training data
        X = self.dataset.X
        sh = np.shape(X)
        new_X = np.zeros((sh[0], sh[1], sh[2]-1))
    ...
    def buildModel (self):
        ...
        self.model.add(LSTM(512, dropout_W=0.2, dropout_U=0.2, stateful=False, return_sequences=True, input_shape=(self.dataset.lookback_window, self.dataset.n_features)))
        self.model.add(Dropout(0.2))
        
        self.model.add(LSTM(128, dropout_W=0.2, dropout_U=0.2, stateful=False, return_sequences=True))
        self.model.add(Dropout(0.2))
        
        self.model.add(LSTM(64, dropout_W=0.2, dropout_U=0.2, stateful=False, return_sequences=True))
        self.model.add(Dropout(0.2))
        
        self.model.add(LSTM(32, dropout_W=0.2, dropout_U=0.2, stateful=False, return_sequences=False))
        self.model.add(Dropout(0.2))
        
        self.model.add(Dense(1, W_regularizer=l2(0.00001), bias=False))
...
my_train = PredictAheadModel (modelname=modelname,
                            modelpath=modelpath, n_features=5, 
                            lookback_window=252, cv_set_size=750,
                            test_set_size=1750)

my_train.dataset.feat_filename_prefix = 'ccy_hist_ext_'
my_train.dataset.feat_filename_suffix = '.txt'
my_train.dataset.feat_names = ['Date', 'Close', 'Open', 'High', 'Low', 'Change']
my_train.dataset.mu_sigma_list = [0,1,2,3]
my_train.dataset.by100_list = []
my_train.dataset.volume_feat_list = []
my_train.dataset.bCenter_y = False
my_train.dataset.last = 3500
...
my_train.dataset.featpath = './datasets/Fx/Parsed'
...
if False:
    ...
    try:
        ...
        #w = my_train.model.layers[4].get_weights ()
        #w[1][0] = 0
        #my_train.model.layers[4].set_weights (w)
    except:
        pass
    my_train.model.layers[4].bias = False
...
for i in range (0):
    my_train.modelname = 'Pure_LSTM_Only_Px_No_Bias_v2'
    f = open (logpath+my_train.modelname,'a')
    random.shuffle(series_list)
    my_train.dataset.min_period_ahead = 5
    my_train.dataset.min_period_ahead = 10
    my_train.dataset.bCenter_y = False
    try:
        ...
        try:
            ...
            if True:
                try:                
                    ...
                    k = 0
                    my_train.loadDataSet(series_list=series_list,begin=0, end=50)
