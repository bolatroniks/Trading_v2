...
@author: Joanna
"""
#here I introduce the following changes:
    #1/ dropna when loading Dataframe
    #2/ concatenate the whole dataset
...
class My_Training ():
    ...
    def train (self, verbose=False):
        ...
        hist = self.model.fit(self.X[:-np.mod(len(self.X),batch_size),:,:], self.y[:-np.mod(len(self.X),batch_size)], batch_size=batch_size, nb_epoch=1)
