...
#here I add a very simple one to check if the RNN is adding any value
...
class My_Training ():
    def __init__ (self, modelname='my_simple_model', modelpath="./", featpath="./", parsedpath="./", labelpath="./", isTraining=True, isTrainOnlyOnce=True):
        #self.model = self.buildModel ()
        self.model = self.buildSimpleModel ()
...
#        return model
    ...
    def buildModel (self):
        ...
        model.add(Dense(256, input_dim=(self.dataset.n_features), init='uniform'))
    ...
    def buildSimpleModel (self,lookback_window=120):
        model = Sequential ()
        
        # here, 20-dimensional vectors.
        model.add(Dense(256, input_dim=(n_features), init='uniform'))
        model.add(Activation('sigmoid'))
        model.add(Dropout(0.5))
        model.add(Dense(128, init='uniform'))
        model.add(Activation('sigmoid'))
    ...
    def evaluateOnLoadedDataset (self, show_plots=True):
        ...
        for k in range(len (self.dataset_list)):
            if np.size(self.dataset_list[k],1) == 6:                    
                ...
                self.ret_array[k], self.long_hits_array[k], self.long_misses_array[k], self.short_hits_array[k], self.short_misses_array[k] = compute_return(self.model,self.cv_X[:-np.mod(len(self.cv_X),batch_size),-1,:], self.cv_y[:-np.mod(len(self.cv_X),batch_size)])
        if show_plots == True:
            fig = plt.figure ()
            #for k in range(len (self.dataset_list)):
    ...
    def train (self, verbose=False):
        ...
        hist = self.model.fit(self.X[300:-np.mod(len(self.X)-300,batch_size),-1,:], self.y[300:-np.mod(len(self.X)-300,batch_size)], batch_size=batch_size, nb_epoch=1)
    ...
    def plotPredictionsByNo (self, series_no, data='cv'):
        ...
        my_pred = self.model.predict(self.cv_X[:,-1,:])
...
def plotSeriesByNo (series_no, path="./", filename_prefix='ccy_hist_feat_', filetype='label', field='Close', df_sel='cv'):
    ...
    if True:
        ...
        plt.plot(cv_df[field].rolling(window=50).mean())
        plt.plot(cv_df[field].rolling(window=100).mean())
        plt.plot(cv_df[field].rolling(window=200).mean())
        plt.show()
    #except:
    if False:
...
def buildSentences (feat_df, labels_df):
    ...
    for i in range(len(feat_df)):
        #feat_df.ix[:,'CDL2CROW':] = 0
        sentences[i,:] = np.array(feat_df.ix[:,'Close':].irow(i).values)
        sentences[:,15:] = 0
        next_chars [i] = labels_df ['Labels'][i]
...
#model_name = 'trained_model_normalized_feat_v1'
model_name = 'my_simple_model_v1'
my_train = My_Training (model_name, model_path, isTrainOnlyOnce=True)
