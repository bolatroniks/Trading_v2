...
def mtf_pca_filter_slow_and_fast (ccy_pair, last_timestamp, **kwargs):
    ...
    ds = Dataset(ccy_pair = ccy_pair,
                 timeframe=fast_timeframe, 
                 from_time=get_from_time(last_timestamp, fast_timeframe), 
                 to_time=last_timestamp,
                 bLoadCandlesOnline = True)
            
    ds.initOnlineConfig ()
    ds.loadCandles ()
    ds.computeFeatures (bComputeHighLowFeatures=False)
    
    if ds.f_df.RSI[-1] > kwargs['RSI_fast_max']:
        return False
    if ds.f_df.RSI[-1] < kwargs['RSI_fast_min']:
        return False        
    
    ds.init_param(instrument = ccy_pair,
                 timeframe=slow_timeframe, 
                 from_time=get_from_time(last_timestamp, slow_timeframe), 
                 to_time=last_timestamp)            
    
    ds.loadCandles ()
    ds.computeFeatures (bComputeHighLowFeatures=False)
    
    if ds.f_df.RSI[-1] > kwargs['RSI_slow_max']:
        return False
    if ds.f_df.RSI[-1] < kwargs['RSI_slow_min']:
        return False
    
    if ds.f_df.close_over_100d_ma_normbyvol[-1] < kwargs['close_over_ma_min']:
        return False
    
    if ds.f_df.close_over_100d_ma_normbyvol[-1] > kwargs['close_over_ma_max']:
        return False
    
    return True

def mtf_pca_filter_slow (ccy_pair, last_timestamp, **kwargs):
# =============================================================================
#     if 'fast_timeframe' in kwargs.keys ():
#         fast_timeframe = kwargs['fast_timeframe']
#     else:
...
def mtf_pca (obj, **kwargs):
    if str(obj).find ('DatasetHolder') < 0:
        ds = obj
    else:
        obj.appendTimeframesIntoOneDataset (higher_timeframe='D',
                                                  lower_timeframe='M15',
                                                  daily_delay = 1)
        ds = obj.ds_dict [obj.instrument + '_M15']
