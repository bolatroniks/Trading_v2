...
static PyMethodDef _C_arraytestMethods[] = {
	...
	{"compute_labels", compute_labels, METH_VARARGS},
...
static PyObject *compute_labels(PyObject *self, PyObject *args)
{
	...
	double **cin, **mout;           // The C matrices to be created to point to the
	                                //   python matrices, cin and cout point to the rows
	                                //   of matin and matout, respectively
	int i,j,n,m;
	double min_stop, vol_denominator, target_multiple;
	double entry_px, stop, target;

	bool bOpenClosePxForEntry, bVaryStopTarget;

	double bad_ret, good_ret;

    //compute_labels (mat, min_stop, vol_denominator, target_multiple)
	/* Parse tuples separately since args will differ between C fcns */
	if (!PyArg_ParseTuple(args, "O!dddii",
		&PyArray_Type, &mat, &min_stop, &vol_denominator, &target_multiple,
            &bOpenClosePxForEntry, &bVaryStopTarget))  return NULL;
	...
	/* Change contiguous arrays into C ** arrays (Memory is Allocated!) */
	mout=pymatrix_to_Carrayptrs(mat);
	...
	for ( i=0; i < m - 1; i++)
	{
        ...
        if (bOpenClosePxForEntry)
            entry_px = mout[3][i]; //use close px
        else
            entry_px = mout[0][i+1]; //use open px of the next candle for entry

        mout[6][i] = entry_px;

        if (bVaryStopTarget)
        {
            stop = max(mout[4][i] / vol_denominator, min_stop);
        ...
        {
            mout[11] [i] = min_stop;
            mout [12] [i] = min_stop * target_multiple;
        }

    //tests if it is a good idea to buy
        for (j =i+1; j < m; j++)
        {
            bad_ret = mout[2][j] / entry_px - 1;
            good_ret = mout[1][j] / entry_px - 1;
            //tests stops
            if (bad_ret <= -stop)
            {
                mout[13][i] = j - i;
    ...
    //tests if it is a good idea to sell
        for (j =i+1; j < m; j++)
        {
            bad_ret = mout[1] [j] / entry_px - 1;
            good_ret = mout[2] [j] / entry_px - 1;
            //tests stops
            if (bad_ret >= stop)
            {
                //stopped out on short position
                mout[10] [i] = mout[1] [j];
                mout[13] [i] = j - i;
                break;
            }
